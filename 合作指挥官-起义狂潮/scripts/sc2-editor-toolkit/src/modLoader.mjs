...
export function loadModIntoStore(modRoot, store) {
  ...
  for (const file of files) {
    ...
    const { entries, consts: fileConsts, issues } = parseCatalogXml(text, file);
    ...
    for (const elem of entries) {
      ...
      if (existing && existing.tag !== elem.tag) {
        fileIssues.push({
          file, line: elem.line, col: elem.col, code: 'XML_ID_CLASS_CONFLICT',
        ...
        fileIssues.push({
          file, line: elem.line, col: elem.col, code: 'XML_DUPLICATE_ID',
          message: `${catalog} "${elem.attrs.id}" 在本 mod 内重复定义` +
            `（首见 ${path.basename(first.file)}:${first.line}，已按引擎语义合并）`,
        });
      }
      if (!seenInMod.has(key)) seenInMod.set(key, { file, line: elem.line });
      store.addEntry(elem);
    }
  }
  return { files, fileIssues, consts, warnings };
