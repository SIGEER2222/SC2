...
function New-LauncherPlan {
    ...
    foreach ($modName in $selectedAlengerMods) {
        ...
        $l2Entries += [PSCustomObject]@{
            ...
            reason = "Alenger on-demand ($alengerBaseName)"
        }
    }
    ...
    # === Generated AlengerBootstrap galaxy injection entry ===
    # The generated LibRebornAdapter_AlengerBootstrap.galaxy is written by the launcher
    # (via alengerBootstrapCli.mjs) to the map Base.SC2Data. Track for plan completeness.
    ...
    # $mapBaseName 提前定义，供 documentRewrite 和 compositionId 共用
    ...
    # === Build plan object ===
    # mapFamily / mapBaseName 已在 documentRewrite 前定义
        ...
        migration = [PSCustomObject]@{
            ...
            knownDebt    = @(
                ...
                [PSCustomObject]@{
                    ...
                    status           = "resolved"
                    reason           = "RESOLVED: AdapterBootstrap removed from CoreRuntime; Alenger adapters now loaded on-demand via RebornMapAdapter generated LibRebornAdapter_AlengerBootstrap.galaxy, driven by alenger-mods.json commanderToAlenger mapping"
                    removalCondition = "N/A (resolved)"
                },
                [PSCustomObject]@{
