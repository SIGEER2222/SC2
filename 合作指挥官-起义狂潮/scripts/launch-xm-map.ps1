...
function Get-ScenarioRoot {
    ...
    if (Test-Path -LiteralPath (Join-Path $workspaceRoot "Mods\XM")) {
        return $workspaceRoot
    }

    $legacyRoot = Join-Path $workspaceRoot "合作指挥官版起义狂潮"
    if (Test-Path -LiteralPath (Join-Path $legacyRoot "Mods\XM")) {
        return $legacyRoot
    }

    throw "Could not locate a scenario root with Mods\\XM under: $workspaceRoot"
}
...
function Set-LiveXMFinalCommanderDependencies {
    ...
    $scenarioRoot = Get-ScenarioRoot
    $sourceDocumentInfo = Join-Path $scenarioRoot "Mods\XM\XMFinal.SC2Mod\DocumentInfo"
...
function Remove-TestBenchBankSections {
    param([string]$Path)
    ...
    $sections = @($xml.SelectNodes("/Bank/Section[starts-with(@name,'TestBench_')]"))
    if ($sections.Count -eq 0) {
        return
    }

    $stamp = "$(Get-Date -Format 'yyyyMMdd-HHmmss-fff')-$([System.Guid]::NewGuid().ToString('N').Substring(0, 8))"
    $backupPath = "$Path.bak-testbench-prune-$stamp"
    Copy-Item -LiteralPath $Path -Destination $backupPath -Force

    foreach ($section in $sections) {
        $null = $bank.RemoveChild($section)
    }

    $xml.Save($Path)
    Write-Host "CAMPAIGN_XCORE_TESTBENCH_PRUNED=path=$Path|removed=$($sections.Count)|backup=$backupPath"
}
...
try {
    ...
    elseif ($EnableXMFinalCommanderDependencyFilter -and ((Split-Path -Leaf $MapPath) -ne "CommanderTestBench.SC2Map")) {
    ...
    foreach ($bankFile in (Get-CampaignXCoreBankPaths -ExplicitPath $BankPath)) {
        Remove-TestBenchBankSections -Path $bankFile
