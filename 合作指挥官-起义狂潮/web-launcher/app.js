...
const CMRE_ORIGINAL_COMMANDER = {
  runtime: 'CMRE',
  displayName: '原版 (CMRE)',
  integrationNote: '仅加载 CMRE mod，不叠加 7vs1 指挥官',
  integrationTone: 'ok',
};
...
function setCmreLogPaths(stdout, stderr) {
  ...
  if (cmreEl.stdoutPathText) {
    cmreEl.stdoutPathText.textContent = stdout ? `标准输出：${stdout}` : '标准输出：-';
    cmreEl.stdoutPathText.title = stdout || '';
  }
  if (cmreEl.stderrPathText) {
    cmreEl.stderrPathText.textContent = stderr ? `错误输出：${stderr}` : '错误输出：-';
...
async function initCmreTab(forceReload = false) {
  ...
  cmreStatus.textContent = '加载中';

  try {
    if (!state.data) {
      cmreStatus.textContent = '等待主数据加载';
      return;
    }
    // 指挥官列表 = 原版 + 7vs1 指挥官
    ...
    cmreStatus.textContent = `已加载 ${cmreState.commanders.length} 个选项, ${cmreState.maps.length} 张地图`;
  } catch (e) {
    cmreStatus.textContent = `加载失败: ${e.message}`;
    if (cmreEl.configIssues) {
      cmreEl.configIssues.textContent = `加载失败: ${e.message}`;
...
function updateCmreSummary() {
  ...
  if (cmreEl.summaryMode) {
    cmreEl.summaryMode.textContent = dryRun ? '只安装' : '启动';
  }
  if (summary) {
    summary.textContent = ready ? '就绪' : '未就绪';
    summary.className = ready ? 'badge status-ok' : 'badge';
...
async function launchCmreGame() {
  ...
  btn.disabled = true;
  stateLabel.textContent = dryRun ? 'DryRun 中...' : '启动中...';
  output.textContent = `启动指挥官: ${cmreState.selectedCommander}\n地图: ${cmreState.selectedMapFile || '默认'}\n模式: ${dryRun ? 'DryRun' : '启动'}\n`;
